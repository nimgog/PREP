...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, NgClass, LogoComponent, CartComponent, TranslocoPipe],
  template: `
    <header
  class="flex justify-center items-center w-full h-[70px] lg:h-[80px] px-4"
  [ngClass]="isTransparent() ? 'bg-transparent' : 'bg-prep-green'"
>
  <div class="flex justify-between items-center w-full max-w-centered-content h-full">
    <div class="flex justify-center items-center w-8 lg:hidden">
      <button class="w-6 h-6" (click)="toggleMenu()">
        <img
          src="/img/main-header/side-menu.svg"
          alt="{{ 'header-side-menu-alt' | transloco }}"
          width="24"
          height="24"
        />
      </button>
    </div>
    <div [class.hidden]="!menuOpen" class="landscape:hidden absolute top-0 left-0 h-svh w-svw z-50 bg-green-900">
      <div class="flex flex-col w-full h-full p-2">
        ...
        <nav class="lg:block">
          <ul class="flex flex-col justify-center gap-x-16 text-prep-beige text-base font-medium uppercase">
            <li (click)="closeMenu()" class="mt-5">
              <a routerLink="/">{{ 'header-home' | transloco }}</a>
            </li>
            <li (click)="closeMenu()" class="mt-5">
              <a routerLink="/shop">{{ 'header-shop' | transloco }}</a>
            </li>
            <li (click)="closeMenu()" class="mt-5">
              <a [routerLink]="survivalKitPageLink">{{ 'header-survival-kit' | transloco }}</a>
            </li>
            <li (click)="closeMenu()" class="mt-5">
              <a routerLink="/blog">{{ 'header-becoming-prepared' | transloco }}</a>
            </li>
            <li (click)="closeMenu()" class="mt-5">
              <a routerLink="/about-us">{{ 'header-about-us' | transloco }}</a>
            </li>
          </ul>
        </nav>
      </div>
    ...
    <nav class="hidden lg:block">
      <ul class="flex justify-center gap-x-16 text-prep-beige text-base font-medium uppercase">
        <li><a routerLink="/">{{ 'header-home' | transloco }}</a></li>
        <li><a routerLink="/shop">{{ 'header-shop' | transloco }}</a></li>
        <li>
          <a [routerLink]="survivalKitPageLink">{{ 'header-survival-kit' | transloco }}</a>
        </li>
        <li>
